    ...
    <template #header>
      <div class="card-header">
        ...
        <div class="right_content">
          <el-form  :inline="true">
            <el-form-item label="分组筛选">
              <el-select filterable v-model="ChartParams.group_id"
                         placeholder="分组筛选"
                         @change="ChangeGroupSelect">
                <el-option
                    v-for="(item,index) in props.groupList"
                    :key="index"
                    :label="item.name"
                    :value="item.id"
                />
...
<script setup>
  ...
  import {reqGetChartByGroupId} from "@/api/chart";
  ...
  const ChangeGroupSelect = (value)=>{
    ChartParams.group_id = value;
    loadChart();
  }
  ...
  const loadChart = async ()=>{
    ...
    loading.value = true;
    let res = await reqGetChartByGroupId(ChartParams.time_dimension, ChartParams.begin_time,ChartParams.end_time,parseInt(ChartParams.group_id));
    loading.value = false;
    let data = res.data;

    let chartTitle = "";
    props.groupList.forEach((val)=>{
      if(val.id == ChartParams.group_id){
        chartTitle = val.name
      }
    })
