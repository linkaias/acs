    ...
    <template #header>
      <div class="card-header">
        ...
        <div class="right_content">
          <el-form  :inline="true">
            <el-form-item label="Method筛选">
              <el-select filterable v-model="ChartParams.method"
                         placeholder="Method筛选"
                         @change="ChangeMethodSelect">
                <el-option
...
<script setup>
  ...
  import {reqGetChartByMethod} from "@/api/chart";
  ...
  onMounted(()=>{
    ...
    if(selectMethodData.length>0){
      ChartParams.method = selectMethodData[0]
    }
    // 渲染图表
    loadChart();
  })
  ...
  const ChangeMethodSelect = (value)=>{
    ChartParams.method = value;
    loadChart();
  }
  ...
  const loadChart = async ()=>{
    ...
    loading.value = true;
    let res = await reqGetChartByMethod(ChartParams.time_dimension, ChartParams.begin_time,ChartParams.end_time,ChartParams.method);
    loading.value = false;
    let data = res.data;

    let chartTitle = ChartParams.method;
