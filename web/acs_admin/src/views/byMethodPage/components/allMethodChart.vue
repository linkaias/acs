  ...
  <el-card class="box-card" shadow="never">
    ...
    <div class="chart" v-loading="loading">
      <el-row :gutter="8">
        ...
        <el-col :span="9">
          <el-card shadow="never">
            <div ref="ref_chart_pie_container" style="height: 400px;" ></div>
          </el-card>
        </el-col>
      </el-row>
...
<script setup>
  ...
  onMounted(()=>{
    ...
    chartPieObj.value =  echarts.init(ref_chart_pie_container.value)
    if(props.groupList.length>0){
      ChartParams.group_id = props.groupList[0].id
    }
    // 渲染图表
    loadChart();
  })
  ...
  const loadChart = async ()=>{
    ...
    loading.value = true;
    let res = await reqGetChartByAllMethod(ChartParams.time_dimension, ChartParams.begin_time,ChartParams.end_time);
    loading.value = false;
    let data = res.data;
    ...
    let legends = [];
    // 遍历data.y_data 对象
    Object.keys(data.y_data).forEach((key)=>{
      legends.push(key)
      yData.push({
        name: key,
        type: 'line',
        data: data.y_data[key]
      })
      //饼图数据
      let count = 0;
      data.y_data[key].forEach((item)=>{
        count += item
      })
      pieData.push({
        value: count,
    ...
    chartAllCountOption.legend.data= legends


    chartAllCountOption.series =yData;
    // 渲染图表
  ...
  const loadPieChart = (data)=>{
    ...
    chartPieObj.value.setOption(pieChartOption);
  }
